...
// Hook to get page details and action handlers
function useDevotionalActions() {
  ...
  const handleNativeOrMenuShare = async () => {
    if (typeof navigator !== "undefined" && navigator.share) {
      try {
        await navigator.share({
          title: pageTitle,
          text: `🙏 Read and recite: "${pageTitle}" on RamHanumanChalisa.com`,
          url: pageUrl,
        });
        return;
      } catch {
  ...
  return {
    ...
    handleTwitterShare,
    handleFacebookShare,
    handleNativeOrMenuShare,
    handlePrint,
    handleDiyaClick,
  };
}

// 1. Desktop Vertical Floating Dock (Left Margin, XL screens only)
  ...
  return (
    ...
    >
      ...
      <div className="relative group flex items-center">
        ...
        >
          <GoogleIcon className="w-5 h-5 filter drop-shadow-xs" />
        </a>
...
// 2. Mobile Friendly Action Section (Shown at the bottom right after post content / FAQ)
export function MobileBottomActionDock() {
  const {
    copied,
    showShareMenu,
    diyaBlessed,
  ...
  return (
    <div className="no-print xl:hidden w-full my-8 p-4 sm:p-5 bg-gradient-to-br from-maroon-deep via-[#4A0E17] to-maroon-deep border-2 border-brass-gold/60 text-stone-ivory rounded-2xl shadow-xl space-y-3.5 relative overflow-hidden">
      {/* Decorative Om Aura */}
      <div className="absolute -top-3 -right-3 text-5xl opacity-10 font-serif select-none pointer-events-none">
        ॐ
      </div>

      <div className="flex items-center justify-between border-b border-brass-gold/30 pb-2.5">
        <div className="flex items-center gap-2">
          <span className="text-2xl filter drop-shadow">🪔</span>
          <div>
            <h4 className="font-serif-display font-bold text-marigold text-sm sm:text-base uppercase tracking-wider leading-snug">
              Share Devotion & Follow Us
            </h4>
            <p className="text-[11px] text-stone-ivory/70 leading-tight">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-2.5">
        ...
        <a
          ...
          title="Add RamHanumanChalisa.com as Google Preferred Source"
          aria-label="Add site to Google Preferred Sources"
          className="flex items-center justify-center gap-2 px-3 py-2.5 bg-white hover:bg-stone-50 text-charcoal-brown rounded-xl border border-brass-gold/50 shadow-sm font-bold text-xs transition-all active:scale-95"
        >
          <GoogleIcon className="w-4 h-4 shrink-0 filter drop-shadow-xs" />
          <span className="truncate">Follow Google</span>
        </a>
        ...
        <button
          ...
          onClick={handleWhatsAppShare}
          title="Share on WhatsApp"
          className="flex items-center justify-center gap-2 px-3 py-2.5 bg-[#25D366] hover:bg-[#20ba59] text-white rounded-xl shadow-sm font-bold text-xs transition-all active:scale-95 cursor-pointer"
        >
        ...
        <button
          type="button"
          onClick={handleCopyLink}
          className={`flex items-center justify-center gap-2 px-3 py-2.5 rounded-xl border text-xs font-bold transition-all active:scale-95 cursor-pointer ${
            copied
              ? "bg-emerald-600 text-white border-emerald-400"
              : "bg-black/30 hover:bg-black/40 text-stone-ivory border-brass-gold/40 hover:text-marigold"
          }`}
        >
          {copied ? (
        ...
        {/* 4. More Share Options */}
        <button
          type="button"
          onClick={handleNativeOrMenuShare}
          className="flex items-center justify-center gap-2 px-3 py-2.5 bg-black/30 hover:bg-black/40 text-stone-ivory rounded-xl border border-brass-gold/40 hover:text-marigold text-xs font-bold transition-all active:scale-95 cursor-pointer"
        >
          <svg className="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <button
          ...
          className={`flex items-center justify-center gap-2 px-3 py-2.5 rounded-xl border text-xs font-bold transition-all active:scale-95 cursor-pointer ${
            diyaBlessed
              ? "bg-marigold text-maroon-deep border-marigold shadow-[0_0_15px_#F59E0B]"
              : "bg-black/30 hover:bg-black/40 text-stone-ivory border-brass-gold/40 hover:text-marigold"
          }`}
        >
          <span className="text-sm">🪔</span>
          <span>{diyaBlessed ? "✨ Diya Lit!" : "Light a Diya"}</span>
        </button>
      </div>

      {/* Social Popup Fallback for Share */}
      {showShareMenu && (
        <div className="pt-2 border-t border-brass-gold/20 flex flex-wrap items-center gap-2 animate-fadeIn">
          <span className="text-[10px] uppercase font-bold text-marigold tracking-wider">Share To:</span>
          <button
            type="button"
            onClick={handleTwitterShare}
            className="px-2.5 py-1 bg-black/40 hover:bg-black/60 rounded text-xs flex items-center gap-1.5 hover:text-marigold cursor-pointer"
          >
          ...
          <button
            ...
            className="px-2.5 py-1 bg-black/40 hover:bg-black/60 rounded text-xs flex items-center gap-1.5 hover:text-marigold cursor-pointer"
          >
            <span className="text-blue-400 font-bold">f</span> <span>Facebook</span>
          </button>
          <button
            type="button"
            onClick={() => {
            ...
            className="px-2.5 py-1 bg-black/40 hover:bg-black/60 rounded text-xs flex items-center gap-1.5 hover:text-marigold cursor-pointer"
          >
            <span className="text-sky-400">✈️</span> <span>Telegram</span>
          </button>
        </div>
      )}
    </div>
